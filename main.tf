...
resource "azurerm_storage_account" "main" {
  ...
  # Security exemption tags - applied early in creation
  tags = {
    SecurityControl = "Ignore"
    purpose         = "storage-with-access-keys"
  }
...
resource "azurerm_windows_function_app" "main" {
  ...
  # Code deployment handled by deploy-function-code.ps1 script (generated by Terraform)
...
resource "local_file" "deploy_function_code_script" {
  content = templatefile("${path.module}/deploy-function-code-template.ps1", {
    resource_group_name   = azurerm_resource_group.main.name
    function_app_name     = azurerm_windows_function_app.main.name
    function_app_url      = "https://${azurerm_windows_function_app.main.default_hostname}"
    function_trigger_url  = "https://${azurerm_windows_function_app.main.default_hostname}/api/HttpTrigger1"
  })
  filename = "${path.module}/deploy-function-code.ps1"
}
